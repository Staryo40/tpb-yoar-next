...
const MenuItem: React.FC<MenuItemProps> = (props) => {
    ...
    return (
        <>
        <label className={menuitemStyles.menu}>
            {/* <input type='checkbox' checked={isOpen} onChange={handleChange}/> */}
        <button ref={buttonRef} onClick={() => setIsOpen(!isOpen)}>
            <span className={menuitemStyles.icon}>{props.icon}</span>
            <span className={menuitemStyles.name}>{props.name}</span>
            <div className={`${menuitemStyles.triangle} ${isOpen ? menuitemStyles.dropped : ""}`} />
        </button>
        {isOpen && <div className={menuitemStyles.dropdown} ref={dropdownContainerRef}>
            <ul>
